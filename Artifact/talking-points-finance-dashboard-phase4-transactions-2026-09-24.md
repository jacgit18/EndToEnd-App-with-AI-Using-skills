...
**Not verified (say these before someone asks):** prod is on migration 0003, not 0004, so none of this is deployed yet; the void confirmation is a plain browser dialog, checked by tests but not by eye in a real browser; the mutation results for the backend other than the lock case are the subagent's report; the filters have been tested at month boundaries in the API but not with real data; no CI.
