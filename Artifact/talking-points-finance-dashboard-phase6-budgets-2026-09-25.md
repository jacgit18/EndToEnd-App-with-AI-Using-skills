...
**Not verified (say these before someone asks):** under test the page ran only in Vitest; the owner then tried it in a real browser and reported it works (no detailed check recorded); the API has only run under pytest against the dev database; prod does not have Phase 6 yet; the UI shows expense categories only while the API accepts any non-archived category.
