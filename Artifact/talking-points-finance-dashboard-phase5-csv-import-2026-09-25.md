...
**Not verified (say these before someone asks):** my real SoFi export (619 rows, 16 accounts) imported once I mapped the columns correctly, but my first two attempts mapped the amount column wrong and rejected all 619 rows with nothing saved (the page doesn't warn about a bad mapping before submit); the automated test files are synthetic; an import can duplicate a hand-entered row (those have no hash); there is no way to undo a whole import, only voiding rows one by one; the 2 MB upload limit is enforced after the server buffers the file, and a hard cap in the reverse proxy isn't added; the backend was written by a subagent, and I reviewed it, re-ran the tests and ran one mutation myself; no CI. Prod status: see the deploy line below.
