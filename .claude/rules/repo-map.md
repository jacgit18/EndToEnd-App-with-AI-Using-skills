...
| Path | What's in it |
|---|---|
| `.claude/skills/<name>/` | The skill library, one flat directory per skill (Claude Code discovers `.claude/skills/<name>/SKILL.md` one level deep; group folders are not discovered). `.claude/skills/INDEX.md` maps each skill to its group. Groups: `AI Engineering`, `Architecture` (+ `Architecture/Data`), `Business`, `Documents`, `Finance`, `Git`, `Health`, `Prompts`, `Research`, `Skill Development`, `Testing`. *(Finance, Health and Research are absent here.)* |
| `.claude/rules/` | This guidance, split into topic files and imported by `CLAUDE.md`. Edit the rule files, not the `CLAUDE.md` list. |
| `.claude/commands/` | Slash commands. `/new-skill <name> [Group]` drives the add-a-skill workflow end to end; `/sync-catalog` is a read-only catalog consistency check. (Directory is `commands`, plural — Claude Code ignores a singular `command/`.) |
