...
| The deep design of one load-bearing decision | `api-interface-style`, `microservices-decision`, `database-architecture`, `data-access-layer`, `access-control-modeling`, `deployment-strategy`, `serverless-execution-model`, `bff-gateway-placement`, `migration-cutover`, `config-and-secrets-management`, `cloud-iam-boundary`, `capacity-estimation` | This skill routes into them for load-bearing decisions and folds their output into the ADR. A single already-isolated decision with its inputs ready goes straight to the specialist. |
