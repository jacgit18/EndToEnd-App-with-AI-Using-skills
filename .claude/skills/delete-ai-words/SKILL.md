...
>
> **Precedence (explicit):** the `software-carpentier-brand` carpenter device (built, joined, load-bearing, structural, framed, reinforced; one use at most) and `explaining-my-work`'s resume dash-clause (the dash-separated outcome clause, which may use an em dash) are exempt from the metaphor-verb ban (framed, layered, woven), the analogy count cap, and the em-dash ban below. When either skill sends text here first, keep those device words and that dash clause intact; strip everything else. If the draft has no brand or resume context, the bans apply in full.
