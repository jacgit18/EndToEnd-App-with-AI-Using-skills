...
- **Error-tracker and APM agent defaults** — a hosted error tracker (Sentry and similar) or vendor APM agent is instrumentation too, and its defaults are tuned for debuggability, not privacy: request bodies, headers/cookies, query strings, DB statements, user IP, stack-frame local variables and breadcrumbs, and sometimes on-screen content (session replay) can leave the machine. For a system handling money, health or credentials, decide each **before** the first event ships: request bodies off, default PII off, local variables off or scrubbed (`include_local_variables` in the Python Sentry SDK), replay/tracing off unless a stated question needs them (also keeps volume inside a free tier). Name the settings, and verify with a test event that the options actually took effect. Example: a personal finance app's Sentry init set `send_default_pii=False`, `max_request_body_size="never"`, `traces_sample_rate=0.0` because request bodies carry transaction descriptions and amounts. Also worth a clause even on a public-content system: user-typed query strings can carry personal data.
