...
- Pre-flight audit of a shipped-or-shipping product against its public commitments (privacy policy, terms, cookie banner, marketing claims), producing a findings register of gaps between what it does and what it discloses or secures.
- Spine: policy-vs-practice disclosure gaps across 15 probes (AI/LLM processing, model training on user data, profiling, subprocessors, retention, data categories, tracking, children's data, transfers, cookies, biometrics, CCPA/CPRA "sale" vs "share", more).
- Secondary: security design anti-patterns (secrets in version control, missing auth, IDOR, PII in logs).
- Flag-only: legal exposure, naming the regime and deferring to counsel.
- Gates on a data inventory.
...
- A bare conceptual question ("what is GDPR Article 22", "what counts as a sale under CPRA") is answered directly.
- Not for code vulnerabilities — `security-review`.
- Not for one proposed change — `change-surface-audit`.
- Not for `design-scoping`, `access-control-modeling`, `cloud-iam-boundary`, `config-and-secrets-management`, `failure-mode-analysis`, `technical-cost-decision`, `resilience-strategy`, or `observability-strategy`.
