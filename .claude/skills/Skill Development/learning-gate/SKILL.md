...
| Learning to write or critique a skill (what makes a description trigger, how a skill body should be structured) | Say what they see wrong in a real skill before the findings are revealed | `skill-static-audit` (a procedure, not a rep gate — cede the coaching level; on a plain "audit this skill" request with no learning intent, hand off with no gate; its escape hatch allows a hint pass when the user is explicitly learning) |
