...
- When `tech-decision-walkthrough` invokes this skill for one decision's recurring-cost axis, size the Cost Surface and dominant line item for *that decision's* candidate options only and hand back — don't expand into pricing the whole stack. When the scope states a cost cap, give the dominant-line figure in a form the walkthrough can compare with it (`tech-decision-walkthrough` owns the cap check; this skill only supplies the number).
