...
description: Use when a technical decision carries a recurring price — choosing between infrastructure or managed services, sizing a system against a stated volume (requests/day, users, uploads, events, GB, tokens), cutting a cloud bill, build-versus-buy, or when the request mentions a monthly spend, budget, runway, or savings target; also when someone proposes spending engineering effort to save money or asks whether something will get expensive. Sizes the bill and finds the dominant line item, assuming and labelling unit prices (usage drivers come from the user). Boundaries: telemetry-stack cost levers are `observability-strategy`. Usage-to-QPS/GB/server sizing is `capacity-estimation` (hands numbers back here); a pure sizing request ("how many servers/QPS at 10M DAU") goes there first. Migration planning is `migration-cutover`. Shed-vs-scale is `resilience-strategy`. Monorepo tooling choice is `microservices-decision`. A bare "X or Y, which is cheaper" between compute primitives goes through `serverless-execution-model` first. Gateway/BFF topology is `bff-gateway-placement`. LLM tier routing is `model-routing-decision`. Agent API integration shape is `api-tooling-selection`. When `tech-decision-walkthrough` invokes this for one decision, size only that decision's options and hand back. Sprint-worth is `ticket-evaluation`; story volume is an input in `user-story-decomposition`.
---
...
- The request contains a volume or rate and asks for a price, a cost, or a choice between options that run continuously — users, requests, uploads, events, GB, tokens, per day or per month. A pure sizing request (servers, QPS, GB/day, what binds first) goes to `capacity-estimation` first, then returns here to price. A ticket or story that merely carries a volume is `ticket-evaluation` / `user-story-decomposition` unless a cost figure is asked for
...
When more than one option is on the table, produce one Surface per candidate (or one compact table with a column per option), never only the option the user named. After the Surfaces, recommend one option, and name:
...
- A bare "X or Y, which is cheaper" where X/Y are compute primitives (Lambda vs Fargate, a function vs a container vs a long-running service) goes through `serverless-execution-model`'s technical-fit gate first — duration, statefulness, and concurrency can rule one out before price is even relevant, and that skill hands the resulting duration/concurrency numbers back here to price.
- Whether a shared API gateway or a backend-for-frontend sits in front of multiple services or client types at all is `bff-gateway-placement`; this skill only prices the chosen layer (managed gateway vs self-hosted) once that topology is settled.
...
- Deciding whether an AI agent's API integration is a plain tool function, an MCP server, or needs a Postman/OpenAPI validation pass first is `api-tooling-selection`, which hands the settled architecture and endpoint count back here to price.
- When `tech-decision-walkthrough` invokes this skill for one decision's recurring-cost axis, size the Cost Surface and dominant line item for *that decision's* candidate options only and hand back — don't expand into pricing the whole stack.
- A volume or rate in the request is a trigger only when a price is being asked for or a recurring-cost choice is at stake: a pure sizing request ("how many servers/QPS at 10M DAU") is `capacity-estimation` first and comes back here to price; whether a ticket is worth the sprint is `ticket-evaluation` and a story's stated volume is just an input in `user-story-decomposition`, neither of which is priced here unless the user asks for a cost figure.
