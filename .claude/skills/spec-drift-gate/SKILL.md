...
- Adding one restated requirement or constraint to a spec that already exists ("put that in the spec", "I keep repeating this") is `context-promotion`; this skill drafts and enforces a spec for a new multi-file build, and `context-promotion` hands over to it when no spec exists and the feature is a build.
