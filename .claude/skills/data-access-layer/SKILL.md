...
description: Gated decision for how application code talks to an already-chosen relational database: raw SQL, query builder, micro-ORM, full ORM, typed codegen, or compile-checked SQL. Use when someone says "ORM or query builder", "should we use an ORM at all", "is Prisma the right call here", "raw SQL vs an ORM for this". NOT `database-architecture` (source of truth, its prerequisite), `relational-modeling`, `index-tuning`, `caching-strategy`, or `api-tooling-selection`.
---
...
- It forces the source-of-truth ADR, the team's SQL fluency, the query-shape mix (plain CRUD vs analytical/reporting vs DB-specific features), the refactor-safety bar, and the learning-vs-velocity priority to be stated by the user before any approach is recommended, then records the outcome as an ADR.
- NOT the where-does-the-schema-live / database-first-vs-code-first-vs-contract-first decision — that is `database-architecture`, this skill's prerequisite; "code-first" there is a source-of-truth call, not the same axis as "ORM vs query builder" here.
...
- NOT tuning or auditing an index set on a live, populated schema — `index-tuning`.
- NOT choosing the migration tool (Alembic / Flyway / Liquibase / dbmate / Atlas) beyond noting that the access-layer choice constrains it.
- NOT whether to skip the application tier entirely and expose the database directly as an API (PostgREST / Hasura / PostGraphile / Supabase) — that is `database-architecture` (source of truth) with `api-interface-style` (the surface).
- NOT the ORM-vs-raw performance of one specific slow query — `index-tuning`, or `problem-solving-gates` (Optimization) if there's a profile in hand.
- NOT the analytical / warehouse model itself — `dimensional-modeling`.
- NOT whether a read should hit the database at all rather than a cache — `caching-strategy`.
