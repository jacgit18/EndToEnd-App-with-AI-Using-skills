...
- Use when the user is stuck before starting — not stuck mid-task, but unable to get in the door of a piece of work — and needs a first move, not a plan or a ranking.
...
- Picks the lowest-resistance entry point (the easiest piece to pick up, even if it is not the most important piece), has the user do one small rep, then forces a re-evaluate from inside the work.
- The premise is that resistance governs whether you start at all and impact governs what you do once immersed — two independent axes; when the easiest entry also happens to be the highest-value piece that is luck, not the plan.
- Claude proposes the entry candidates here (a stalled user should not have to generate them) and picks by resistance only; it does NOT rank by value at this stage.
- Has an exit condition against permanent avoidance: after one completed rep or roughly 20 minutes, the re-evaluate step is mandatory, and after two entry reps with no re-evaluation it is forced.
...
- NOT a plain "I'm building X, where do I start" with no stall signal (avoidance, overwhelm) — that is `design-scoping`'s front door.
...
- NOT for someone who knows where to start and wants it done (execution — just do it) or a reference lookup.
