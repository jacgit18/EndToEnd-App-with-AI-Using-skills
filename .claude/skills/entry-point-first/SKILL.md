...
description: Use when the user is stuck before starting — not stuck mid-task, but unable to get in the door of a piece of work — and needs a first move, not a plan or a ranking. Triggers on "I don't know where to start", "can't get started", "I keep putting this off", "overwhelmed by this codebase", "this is too big to even begin", "where do I even begin with this", "I've been staring at this all morning", "just inherited this repo and I'm lost". Picks the lowest-resistance entry point (the easiest piece to pick up, even if it is not the most important piece), has the user do one small rep, then forces a re-evaluate from inside the work. The premise is that resistance governs whether you start at all and impact governs what you do once immersed — two independent axes; when the easiest entry also happens to be the highest-value piece that is luck, not the plan. Claude proposes the entry candidates here (a stalled user should not have to generate them) and picks by resistance only; it does NOT rank by value at this stage. Has an exit condition against permanent avoidance: after one completed rep or roughly 20 minutes, the re-evaluate step is mandatory, and after two entry reps with no re-evaluation it is forced. NOT the prioritization call itself — once the user is moving and the question becomes "which of these matters most / immediate payoff vs long-term value", that is `ticket-evaluation` (a ticket), `user-story-decomposition` (an epic or feature to break down), or `problem-solving-gates` Options Generator (a decision where the user has candidates and a lean); this skill hands off to them and does not restate their logic. NOT `problem-solving-gates` Rubber Duck for a live bug or symptom ("where do I even start" on a broken thing is `debugging-layer-selection` first, then Rubber Duck). NOT `tech-decision-walkthrough` (choosing a build's stack) and NOT `incremental-build-pacing` (slowing the delivery of an already-specced build) — the user there is not stalled, they are mid-flow. NOT `learning-gate`, which classifies whether the user wants to learn and sets the assistance ceiling; a stalled start is an activation problem, not a learning-vs-execution one, and this skill routes from its Step 3 table. NOT a plain "I'm building X, where do I start" with no stall signal (avoidance, overwhelm) — that is `design-scoping`'s front door. NOT `spec-drift-gate` — if the work is a multi-file build with no written spec the entry rep is at most "write three bullets of the spec", not code. NOT for someone who knows where to start and wants it done (execution — just do it) or a reference lookup.
---
