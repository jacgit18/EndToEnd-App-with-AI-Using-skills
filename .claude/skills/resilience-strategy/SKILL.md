...
- **Per-invocation retry / dead-letter contract of one async unit of work** → `serverless-execution-model`.
...
- A gated decision for how a service or request path protects itself under overload and dependency failure — load shedding with priority tiers, rate limiting (per-client vs global, token-bucket / sliding-window), concurrency limiting and backpressure, circuit breakers on outbound calls, timeout + retry-budget + backoff-with-jitter policy, bulkhead / pool isolation, and graceful degradation with fallbacks — plus where each control sits (edge / API gateway / service mesh / in-process).
- Use when someone says "we need rate limiting", "add a circuit breaker", "the service falls over under load", "a slow dependency took everything down", "one noisy client starved everyone", "how do we degrade gracefully", "we need backpressure", "load shedding", or proposes a resilience mechanism and wants it checked.
- It forces the user to state the triggering incident or capacity ceiling, what resource binds first, the priority tiers of the traffic, the client model, and the downstream dependencies before any mechanism is recommended, then records the outcome as an ADR.
- Not for detecting or alerting on overload — that is `observability-strategy`, whose SLIs this skill consumes.
- Not for scaling the datastore (replicas, sharding, DB connection pooling) — that is `data-tier-operations`.
- Not for the cache design itself, including a cache-key stampede / thundering herd on key expiry — that is `caching-strategy`; though "fall back to a cached response" is a degradation mechanism this skill names.
- Not for diagnosing one endpoint that fell over last night — that is `problem-solving-gates` (Rubber Duck).
- Not for whether a service mesh exists at all — that is `service-mesh-adoption`; this skill assumes a mesh's presence or absence as a given fact and only decides what runs where once that's settled.
- Not for interpreting live utilization or latency numbers to decide whether a pressure is real — that is `reliability-math`, whose reading feeds this skill; a-priori sizing is `capacity-estimation`.
- Not for enumerating the whole failure surface — that is `failure-mode-analysis`.
- Not for per-invocation retry / dead-letter contracts of one asynchronous unit of work — that is `serverless-execution-model`.
- Not for whether a gateway/BFF layer exists at all — that is `bff-gateway-placement`, which hands the "centralize here" decision to this skill; edge/WAF/DDoS network placement is `cloud-iam-boundary`.
- Not for how many services exist — that is `microservices-decision`.
- Not for the dollar cost of a mechanism — that is `technical-cost-decision`.
- Not for what a change breaks elsewhere before it ships — that is `change-surface-audit`.
