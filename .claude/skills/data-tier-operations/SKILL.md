...
- Not for executing a live move to the new topology (backfill + cutover + rollback window) — that is `migration-cutover`, which this skill hands the move to once the target topology is chosen.
...
- Not for adding, revising, or auditing indexes on one slow query — that is `index-tuning`.
- Not for designing a cache in front of the store — that is `caching-strategy`.
- Not for the table design itself (normal form, keys, first-cut index list) — that is `relational-modeling`.
- Not for choosing the store or where the source of truth lives — that is `database-architecture`.
- Not for projecting QPS/storage/capacity numbers for a store that does not exist yet — that is `capacity-estimation`.
