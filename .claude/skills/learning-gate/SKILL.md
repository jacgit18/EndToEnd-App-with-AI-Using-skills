...
| Choosing a build's technologies out loud, one decision at a time, with alternatives, tradeoffs and a recommendation per decision — "walk me through the stack" | State the build's scope + name the decisions to walk; give a lean per decision or say there's none | `tech-decision-walkthrough` (a coached procedure, not a rep gate; not Options Generator or `system-design-communication` Mode 3 — see `step3-rows.md` for the distinctions) |
| Stuck before starting — can't get in the door ("I don't know where to start", "can't get started", "overwhelmed by this codebase", putting it off) | Do one small, reversible entry rep chosen for lowest resistance (Claude proposes the candidates — a frozen user isn't asked to invent them), then say what they noticed | `entry-point-first` (a gate on resistance, not on learning — hand off, don't ask the Step 1 ambiguous-intent question first; the value-vs-payoff call it defers to `ticket-evaluation` / `user-story-decomposition` / Options Generator; once the user is moving and wants a concept taught, they come back here) |
| Learning to write or critique a skill (what makes a description trigger, how a skill body should be structured) | Say what they see wrong in a real skill before the findings are revealed (if no skill is named yet, teach the description principles directly — triggers, carve-outs, reciprocity — and offer `skill-static-audit` on a real one; don't invoke the audit without a target) | `skill-static-audit` (a procedure, not a rep gate — cede the coaching level; on a plain "audit this skill" request with no learning intent, hand off with no gate; its escape hatch allows a hint pass when the user is explicitly learning) |
| Debugging | Form a hypothesis | `problem-solving-gates` (Rubber Duck) — except a live symptom where the open question is *which tool/layer to look in* (DevTools vs backend observability vs Wireshark): that is `debugging-layer-selection`, and Rubber Duck does not gate it |
...
| Architecture decision | Name constraints + a lean | `problem-solving-gates` (Options Generator), `tech-decision-walkthrough` (whole-build stack), or the owning Architecture skill (`database-architecture`, `microservices-decision`, `api-interface-style`, `observability-strategy`, `resilience-strategy`, `deployment-strategy`, and others) — full list with each skill's scope in `step3-rows.md`; read it when unsure which one owns the decision |
...
| Testing — how a DB-touching test gets its database (Testcontainers / shared DB / in-memory / mock / GUI) | Name the test level, what prod code does at the DB, whether CI can run a real DB, and the shared-instance flakiness cost | `database-test-tooling` (a gate; its prerequisite is `test-strategy`) |
...
**When a skill in the "Defer to" column also fires on this request, hand off — don't stack.** That skill runs its own precondition gate (or, for procedures such as `index-tuning`, `failure-mode-analysis`, `reliability-math`, `disclosure-gap-audit`, `codebase-file-orientation`, `test-case-discovery`, `tech-decision-walkthrough`, just cedes the coaching level). `step3-rows.md` lists each skill's gate questions; read it if you need to know what the domain skill will ask. Classify the intent, set the ceiling from Step 4, and let the domain skill own the gate. Asking the learning rep questions *and* the domain skill's gate questions in the same turn is the stacking failure — two walls of preconditions for one request.
...
- Run this skill's rep gate on top of a domain skill's own gate when both match the request. Classify the intent, then hand off — the domain skill (`database-architecture`, `relational-modeling`, `data-access-layer`, `dimensional-modeling`, `data-tier-operations`, `index-tuning`, `caching-strategy`, `observability-strategy`, `resilience-strategy`, `capacity-estimation`, `failure-mode-analysis`, `reliability-math`, `design-scoping`, `disclosure-gap-audit`, `migration-cutover`, `deployment-strategy`, `api-interface-style`, `api-tooling-selection`, `microservices-decision`, `bff-gateway-placement`, `service-mesh-adoption`, `config-and-secrets-management`, `technical-cost-decision`, `cloud-iam-boundary`, `access-control-modeling`, `serverless-execution-model`, `user-story-decomposition`, `problem-journal`, `test-strategy`, `coverage-policy`, `test-practice-gate`, `test-case-discovery`, `database-test-tooling`, `history-integration-strategy`, `problem-solving-gates`, `code-review`, `model-routing-decision`, `codebase-file-orientation`, `incremental-build-pacing`, `tech-decision-walkthrough`, `entry-point-first`) owns the precondition questions.
