...
| Persisting the increment map across a context reset or session end | `session-handoff` | Not this skill's job to trigger, but when it fires mid-build, its handoff file is where the live increment map should land — see `increment-delivery.md`'s "Keeping the map visible". Read that file's list as canonical on resume, not from memory of the build's pattern. |
