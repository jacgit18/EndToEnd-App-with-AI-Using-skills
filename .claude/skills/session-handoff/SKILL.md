...
If `incremental-build-pacing` is mid-flight, item 6 (Next steps) **is** the
...
- Durable context that stays true across sessions (standing rules, a feature's requirements, how the user likes to work) is not a handoff — that is `context-promotion`, which writes it once to a rules file, a spec, or memory after the user confirms.
