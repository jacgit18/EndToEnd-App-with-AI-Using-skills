...
- The user needs to **propagate row changes out of the database** — keep a search index, a cache, a read model, a warehouse, or a downstream service in sync — and asks about dual-write vs transactional outbox vs change data capture (Debezium, logical replication).
...
- **"we'll just dual-write to Postgres and Elasticsearch / the cache / the warehouse"** — what happens when the second write fails after the first commits? Dual-write has no atomicity and silently drifts. The transactional outbox (one local commit, async relay) or change data capture off the WAL/binlog (Debezium, logical replication) gives you exactly-one source of change with at-least-once delivery — pick CDC when you can't change the writing app, outbox when you can.
...
- `consistency-and-transactions.md` — isolation levels and the anomalies each prevents, optimistic vs pessimistic locking, distributed-transaction and change-propagation patterns (2PC, Saga, transactional outbox, change data capture, eventual — and why not dual-write), replication lag and read-your-writes, connection pooling.
