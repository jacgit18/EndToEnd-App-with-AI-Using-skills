...
- **Gateway/BFF topology and where auth terminates** — whether a gateway or BFF fronts the clients, and where in the request path a check is terminated → `bff-gateway-placement`, which routes the "does this need to be internet-facing" question here.
- **Auditing an existing product's disclosures and controls** → `disclosure-gap-audit`, which hands over-permissive-grant findings here.
