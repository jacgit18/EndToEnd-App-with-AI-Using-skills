...
- Use when a feature, epic, or requirement needs to become concrete backlog items — "write user stories for X", "break this epic down", "what's the acceptance criteria for X", "should this be a use case or a user story", "turn these requirements into a backlog", or a named actor + action that needs a proper As-a/I-want/so-that with acceptance criteria.
