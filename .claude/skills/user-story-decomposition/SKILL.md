...
- Also handles "break this into stories I can document as I ship them" / "size these so I can post about each one" by adding a documentation-cadence lens; the writeup itself is `explaining-my-work`, only once a story is closed.
- A bare feature name with no breakdown routes here if the ask is "turn this into stories," or to `design-scoping` first if the ask is to scope a whole system around it.
