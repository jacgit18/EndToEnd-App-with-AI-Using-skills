...
- Slice 5c done (2026-09-25): deployed to prod on the owner's explicit "deploy to prod" (backup `finance-20260925-143311.sql.gz` first, then `prod.sh up -d --build`). Owner confirmed on prod: dashboard chart renders and a CSV upload worked. No detailed per-figure record kept.
