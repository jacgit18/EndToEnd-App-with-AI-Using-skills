...
| 2026-09-25 | **Phase 6 (S6 budgets) built: backend + Budgets page.** Decisions inside S6's intent: amount must be **> 0** with 2 decimals as a string ("no row = no budget", so clearing deletes the row and DELETE on a missing row is 404); month is `YYYY-MM`, year 0000 rejected; PUT is an upsert (`INSERT .. ON CONFLICT DO UPDATE`); an **archived category is 409**, unknown is 404 (mirrors transactions); **copy-forward never overwrites** existing rows, skips archived categories and returns `{copied, skipped}`. The UI lists expense categories only (the API accepts any non-archived category). No migration: the `budgets` table has existed since 0001. Found by tests: `rowcount` is -1 for INSERT..SELECT..ON CONFLICT, so copy-forward counts with RETURNING. Owner reported the Budgets page working in a browser 2026-09-25 (no per-action detail recorded); not on prod. | Amend: > 0 rule, archived-409 and copy-forward semantics are new decisions inside S6; the rest is S6 as written. |
| 2026-09-25 | **Phase 7 decisions (ADR-0017, ADR-0018).** Charting library = **Visx** (SVG, explicit sizes; overrides the Recharts lean in ADR-0008/spec); **no component library** (keep current approach). Aggregation endpoints (owner delegated): `GET /api/dashboard?month=` and `GET /api/dashboard/trend`, raw SQL, money as strings, same month validation as budgets. | Not an amendment to S7's scope; resolves items the spec deferred to build-time. |
| 2026-09-25 | **Phase 7 spec written** (`docs/phase7-spec.md`). Income/expense are defined **by category kind** with refunds netted (not by sign of amount); uncategorized rows are summed and placed by the sign of their net; net always equals the plain sum of the month's amounts. Budget-only and spend-only categories both appear. | Not an amendment: definitions inside S7's intent, confirmed by the owner. |
